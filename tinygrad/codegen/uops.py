...
class UPat:
  ...
  allow_len: List[int] = field(default_factory=list)

def _match(uop:UOp, pattern:UPat, store:Dict[str, UOp]) -> bool:
  if pattern.name:
      if pattern.name in store and store[pattern.name] != uop: return False
      store[pattern.name] = uop
  if pattern.arg:
      if uop.arg != pattern.arg: return False
  if pattern.dtype:
      if isinstance(pattern.dtype, set):
        if uop.dtype not in pattern.dtype: return False
      elif uop.dtype != pattern.dtype: return False
  if pattern.uop:
      if isinstance(pattern.uop, set):
        if uop.uop not in pattern.uop: return False
      elif uop.uop != pattern.uop: return False
  if pattern.vin:
      ...
      for vp in itertools.permutations(pattern.vin) if isinstance(pattern.vin, list) else ([pattern.vin] if isinstance(pattern.vin, tuple) else [(pattern.vin,)*len(uop.vin)]):
        if len(uop.vin) != len(vp) and (len(uop.vin) not in pattern.allow_len): return False
